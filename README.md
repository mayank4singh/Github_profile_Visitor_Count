...
Real human opens github.com/YOUR-USERNAME
    ...
    GitHub's camo proxy fetches the image
        ...
        Flask server receives request
              ...
              │   ├── "github-camo" → real visit → INSERT into SQLite
...
For a visitor counter, SQLite is the correct choice. Zero configuration, zero network hops, zero cost. Every read and write is a local file operation — faster than a remote database for simple `INSERT` and `COUNT` queries.
...
├── database.py         # SQLite — init, record_visit, get_total, get_daily_counts
├── svg_generator.py    # Builds the SVG response string from count data
├── requirements.txt    # Flask, Gunicorn
├── render.yaml         # Render.com deployment config
└── .gitignore          # Excludes venv/, counter.db, __pycache__/
```
...
### `database.py`
Manages the SQLite connection. Three core functions — `init_db()` creates the visits table on startup, `record_visit()` inserts a new row with the current UTC timestamp, `get_daily_counts()` aggregates visits by day for the chart, and `get_first_visit_date()` returns the earliest recorded visit for the date range label.
...
**Built with ❤️ and lots of ☕ by [Mayank Singh](https://github.com/mayank4singh)**
